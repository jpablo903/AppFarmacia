...
public class PurchasesDao {
    ...
    public List listPurchasesDetailQuery(int id) {
        ...
        String query = "SELECT pu.created, pude.purchase_price, pude.purchase_amount, pude.purchase_subtotal, su.name \n" +
                        "AS suppliers_name, pro.name AS product_name, em.full_name FROM purchases pu INNER JOIN purchase_details pude ON\n" +
                        " pu.id = pude.purchase_id INNER JOIN products pro ON pude.product_id = pro.id INNER JOIN \n" +
                        " suppliers su ON pu.supplier_id = su.id INNER JOIN employees em ON pu.employee_id = em.id WHERE pu.id=?";
